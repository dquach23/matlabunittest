classdef HtmlBackend < handle
    ...
    methods
        ...
        function addStatusBadge(obj, status, inlineLabel)
            %ADDSTATUSBADGE  v1.6 -- emit a colored pill badge for a status.
            %   When INLINELABEL is provided, the badge sits inline with
            %   the supplied label (e.g. "Status: [Pass]"); otherwise the
            %   badge stands alone in its own paragraph.
            arguments
                obj
                status      (1,:) char
                inlineLabel (1,:) char = ''
            ...
            badge = sprintf( ...
                ...
                fill, text, ...
                autotest.report.backends.HtmlBackend.esc(label));
            if isempty(inlineLabel)
                obj.BodyParts(end+1) = string(['<p>' badge '</p>']);
            else
                obj.BodyParts(end+1) = string(['<p>' ...
                    autotest.report.backends.HtmlBackend.esc(inlineLabel) ...
                    ' ' badge '</p>']);
            end
        end

        function addCalloutBox(obj, value, caption)
            %ADDCALLOUTBOX  v1.6 -- executive-summary callout (big number + caption).
            arguments
                obj
                value   (1,:) char
                caption (1,:) char
            end
            obj.BodyParts(end+1) = string(['<aside class="callout">' ...
                '<div class="callout-value">' ...
                autotest.report.backends.HtmlBackend.esc(value) '</div>' ...
                '<div class="callout-caption">' ...
                autotest.report.backends.HtmlBackend.esc(caption) '</div>' ...
                '</aside>']);
        end

        function addFailureCard(obj, info)
            %ADDFAILURECARD  v1.6 -- single failure entry in the new Failed Tests section.
            %   INFO struct fields:
            %       .Class .Method .Identifier .Message .Stack (cellstr)
            %       .Explanation .Source (optional)
            arguments
                obj
                info struct
            end
            fname = autotest.report.SectionBuilder.fieldOrDefault(info, ...
                'Class', '(unknown class)');
            ...
            parts = strings(0,1);
            parts(end+1,1) = "<article class=""failure-card"">";
            header = sprintf('%s.%s', fname, mname);
            if ~isempty(src)
                header = sprintf('%s &middot; %s', header, src);
            end
            parts(end+1,1) = string(['<h4>' ...
                autotest.report.backends.HtmlBackend.esc(header) '</h4>']);
            if ~isempty(ident)
                parts(end+1,1) = string(['<p class="failure-ident"><strong>' ...
                    autotest.report.backends.HtmlBackend.esc(ident) ...
                    '</strong></p>']);
            end
            parts(end+1,1) = string(['<p class="failure-msg">' ...
                autotest.report.backends.HtmlBackend.esc(msg) '</p>']);
            if ~isempty(stack)
                stackHtml = '';
                for k = 1:numel(stack)
                    stackHtml = [stackHtml '<li>' ...
                        autotest.report.backends.HtmlBackend.esc(stack{k}) ...
                        '</li>']; %#ok<AGROW>
                end
                parts(end+1,1) = string(['<ol class="failure-stack">' ...
                    stackHtml '</ol>']);
            end
            if ~isempty(expl)
                parts(end+1,1) = string(['<div class="failure-explain">' ...
                    '<strong>Plain-English explanation.</strong> ' ...
                    autotest.report.backends.HtmlBackend.esc(expl) ...
                    '</div>']);
            else
                parts(end+1,1) = "<div class=""failure-explain failure-unclassified"">" + ...
                    "<em>Cause not classified -- investigate manually.</em></div>";
            end
            parts(end+1,1) = "</article>";
            obj.BodyParts(end+1) = strjoin(parts, newline);
        end
        ...
        function addTable(obj, headers, rows, ~)
            ...
            for r = 1:size(rows, 1)
                ...
                for c = 1:size(rows, 2)
                    ...
                    cls = '';
                    cellHtml = '';
                    % v1.6: detect [STATUS:<name>] sentinel and render
                    % as a badge pill.  Lets Appendix A pass statuses
                    % through the normal addTable path while still
                    % getting the badge treatment.
                    statusTok = regexp(char(val), ...
                        '^\[STATUS:([A-Za-z]+)\]$', 'tokens', 'once');
                    if ~isempty(statusTok)
                        ...
                        cellHtml = autotest.report.backends.HtmlBackend.esc(val);
                    end
                    body = [body '<td' cls '>' cellHtml '</td>']; %#ok<AGROW>
                end
                body = [body '</tr>']; %#ok<AGROW>
            end
    ...
    methods (Static, Access = private)
        ...
        function s = classificationBanner(level, where)
            % v1.6: per DoDM 5200.01 V2, UNCLASSIFIED docs use a plain-
            % text centred banner -- no coloured background block.  Higher
            % classifications retain the v1.4 CAPCO-colored block.
            label = upper(strtrim(level));
        ...
        function css = styleSheet()
            ...
            % v1.6: serif body (Georgia), sans headings (Calibri/Aptos),
            % gold accent rule under h2, alternating row shading, badge
            % pills, callout boxes, failure cards.
            css = [ ...
                ':root{--text:#1F2937;--text2:#4B5563;--accent:#B45309;' ...
                '--fail:#991B1B;--codefill:#F7FAFC;--hdrshade:#E7E6E6;' ...
                '--metashade:#F2F2F2;--border:#BFBFBF;--altrow:#F7FAFC;' ...
                '--pass-fill:#D1FAE5;--pass-text:#065F46;' ...
                '--fail-fill:#FEE2E2;--fail-text:#991B1B;' ...
                '--inc-fill:#FEF3C7;--inc-text:#92400E;' ...
                '--skip-fill:#E5E7EB;--skip-text:#374151;}' newline ...
                '*{box-sizing:border-box}' newline ...
                'html,body{margin:0;padding:0;background:#FFFFFF;' ...
                'color:var(--text);font-family:Georgia,"Times New Roman",serif;' ...
                'font-size:14px;line-height:1.6}' newline ...
                ...
                'font-family:Calibri,"Aptos","Segoe UI",Arial,sans-serif}' newline ...
                '.banner-bottom{margin-top:32px}' newline ...
                'h1,h2,h3,h4{font-family:Calibri,"Aptos","Segoe UI",Arial,sans-serif}' newline ...
                ...
                'text-align:center;font-size:18px;margin:0 0 24px;' ...
                'font-family:Georgia,"Times New Roman",serif}' newline ...
                'h2.lvl1{color:var(--text);font-size:22px;' ...
                'border-bottom:1px solid var(--accent);' ...
                'padding-bottom:4px;margin:36px 0 14px}' newline ...
                ...
                'p{margin:8px 0 12px;text-align:justify}' newline ...
                'p.blank{margin:18px 0}' newline ...
                'hr.accent-rule{border:0;border-top:1px solid var(--accent);' ...
                'margin:18px auto;width:60%}' newline ...
                'table{border-collapse:collapse;width:100%;margin:8px 0 20px}' newline ...
                'table.data{border-top:1px solid var(--border);border-bottom:1px solid var(--border)}' newline ...
                'table.data th,table.data td{border-bottom:1px solid var(--border);' ...
                'padding:6px 10px;text-align:left;vertical-align:top;font-size:13px;' ...
                'font-family:Calibri,"Aptos","Segoe UI",Arial,sans-serif}' newline ...
                'table.data th{background:var(--hdrshade);font-weight:bold;color:var(--text2)}' newline ...
                'table.data tbody tr:nth-child(even){background:var(--altrow)}' newline ...
                ...
                'font-family:Calibri,"Aptos","Segoe UI",Arial,sans-serif}' newline ...
                'table.kv td{padding:6px 12px;border:1px solid var(--border);font-size:13px;' ...
                'font-family:Calibri,"Aptos","Segoe UI",Arial,sans-serif}' newline ...
                ...
                'span.badge{display:inline-block;font-family:Calibri,"Aptos",sans-serif;' ...
                'font-weight:bold;font-size:11px;letter-spacing:0.04em;padding:2px 10px;' ...
                'border-radius:10px;vertical-align:middle;text-transform:uppercase}' newline ...
                'aside.callout{display:inline-block;border:1px solid #E5E7EB;' ...
                'border-left:4px solid var(--accent);background:var(--codefill);' ...
                'padding:14px 22px;margin:8px 8px 8px 0;min-width:160px;' ...
                'vertical-align:top}' newline ...
                'aside.callout .callout-value{font-family:Calibri,"Aptos",sans-serif;' ...
                'font-size:28px;font-weight:bold;color:var(--accent);line-height:1}' newline ...
                'aside.callout .callout-caption{font-family:Calibri,"Aptos",sans-serif;' ...
                'font-size:12px;color:var(--text2);margin-top:4px;text-transform:uppercase;' ...
                'letter-spacing:0.05em}' newline ...
                'article.failure-card{border:1px solid #E5E7EB;border-left:4px solid var(--fail);' ...
                'background:#FFFFFF;padding:14px 18px;margin:12px 0;border-radius:3px}' newline ...
                'article.failure-card h4{margin:0 0 4px;font-size:14px;' ...
                'font-family:Calibri,"Aptos",sans-serif}' newline ...
                'article.failure-card p.failure-ident{margin:4px 0;font-size:12px;' ...
                'font-family:Consolas,monospace;color:var(--fail)}' newline ...
                'article.failure-card p.failure-msg{margin:6px 0;font-size:13px}' newline ...
                'article.failure-card ol.failure-stack{margin:6px 0 6px 22px;padding:0;' ...
                'font-family:Consolas,monospace;font-size:11px;color:var(--text2)}' newline ...
                'article.failure-card .failure-explain{background:var(--codefill);' ...
                'padding:8px 12px;margin-top:8px;border-radius:3px;font-size:13px}' newline ...
                'article.failure-card .failure-unclassified{color:var(--text2)}' newline ...
