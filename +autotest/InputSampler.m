classdef InputSampler
    ...
    methods (Static)
        ...
        function s = makeCase(label, callExpr, kind)
            % NOTE: DropStatefulWrap MUST be part of the prototype.  Three
            % helpers in autotest.TestWriter (applyLiveKeySubstitution,
            % applyContainerSubstitution, markDropStatefulWrapIfClean) set
            % `smartCase.DropStatefulWrap = true` on a SINGLE element of
            % the struct array.  If the field is not already present,
            % MATLAB throws "Subscripted assignment between dissimilar
            % structures" on the next `smart(s) = smartCase` write because
            % the source has more fields than the existing array slot.
            s = struct( ...
                'Label', label, ...
                'Expr', callExpr, ...
                'Kind', kind, ...
                'Args', {{}}, ...
                'DropStatefulWrap', false);
        end

        function s = emptyCase()
            % Field set must match makeCase() exactly -- see note above.
            s = struct( ...
                'Label', {}, ...
                'Expr', {}, ...
                'Kind', {}, ...
